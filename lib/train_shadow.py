...
from lib.models.shadow_model import Models
from lib.dataset.shadow_dataset import My3DDataset
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/shadow.yaml', help='Path to the config file.')
    ...
    train_loader = DataLoader(dataset=dataset, batch_size=config['batch_size'], shuffle=True, pin_memory = True,num_workers=config['nThreads'])
    ...
    while True:
        for it,out_data  in enumerate(train_loader):
            ...
            p_,mask_,light,shadow,r_light,r_shadow=out_data
            trainer.update_learning_rate()
            with Timer("Elapsed time in update: %f"):
                # Main training code
                if (config['models_name'] == 'shadow'):
                    trainer.gen_update(p_,mask_,light,shadow,r_light,r_shadow,config)
